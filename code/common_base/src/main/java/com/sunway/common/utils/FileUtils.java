...
public class FileUtils {
    ...
    public static final String APPS_ROOT_DIR = getExternalStorePath() + File.separator + BaseApplication.Companion.getContext()
            .getPackageName();
    ...
    public static LinkedList<File> listLinkedFiles(String strPath) {
        ...
        File[] mFile = dir.listFiles();
        LinkedList<File> list = null;
        if (dir.exists() && mFile != null && mFile.length > 0) {
            list = new LinkedList<>();
            Collections.addAll(list, mFile);
        }
        return list;
    }
